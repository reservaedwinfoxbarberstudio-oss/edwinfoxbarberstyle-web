...
const CACHE_ASSETS  = 31536000;// Edwin Fox Barber Style — Cloudflare Worker
// Arquitectura: R2 (assets/media/data) + Cloudflare Images URL + APIs
...
// ── IMAGEN CON TRANSFORMACIÓN VÍA CLOUDFLARE IMAGES URL ────────────────────
async function serveImage(url, env) {
  ...
  const hash    = (env && env.IMAGES_ACCOUNT_HASH) ? env.IMAGES_ACCOUNT_HASH : '';

  if (!hash) {
    // Sin hash: servir directo desde R2
    try {
      const obj = await env.EDWIN_MEDIA.get(key);
      if (obj) {
        return new Response(obj.body, {
          headers: {
            ...
            'Cache-Control': 'public, max-age=' + CACHE_MEDIA
          }
        });
      }
    } catch (_) {}
  ...
  // Con hash: usar Cloudflare Images URL interface
  const imgUrl = 'https://imagedelivery.net/' + hash + '/' + key + '/public'
